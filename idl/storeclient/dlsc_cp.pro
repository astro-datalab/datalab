...
;
; USAGE:
;  IDL>res = dlsc_cp(token,'file.txt','file2.txt')
;
; By D. Nidever   June 2017, translated from storeClient.py
;-
 
function dlsc_cp,token,fr,to,verbose=verbose
...
if not dlsc_hasmeta(fr) then begin
  r = dlsc_getfromurl("/cp?from="+src+"&to="+dest, token)
  return,r
endif else begin
  ...
  for i=0,nfiles-1 do begin
    ...
    r = dlsc_getfromurl("/cp?from="+f+"&to="+to_fname, token)
    fnum += 1
